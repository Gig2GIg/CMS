      ...
      <section v-if="loaded">
        ...
        <div class="card">
          <div class="card-content">
            <div class="columns" v-if="renters.length">
...
export default {
  ...
  computed: {
    ...mapState('renters', ['renters', 'isLoading']),
    ...mapGetters('renters', ['search']),
  ...
  methods: {
    ...mapActions('renters', ['fetch', 'broadcast', 'notify', 'destroy']),
    ...
    confirmNotification(renter) {
      this.$dialog.prompt({
        ...
        onConfirm: (value) => this.sendNotification(renter, value),
      });
    },

    confirmDelete(renter) {
      this.selectedRenter = renter;

      this.$dialog.confirm({
        message: `Are you sure you want to delete "${this.selectedRenter.name}"?`,
        confirmText: "Yes, I'm sure",
        type: 'is-danger',
        hasIcon: true,
        onConfirm: this.deleteRenter,
      });
    },
    ...
    async sendNotification(renter, message) {
      await this.notify({ renter, message });
    },

    async deleteRenter() {
      await this.destroy(this.selectedRenter);
    },
  },
  async created() {
    await this.fetch();
