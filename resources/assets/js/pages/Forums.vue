<template>
  <div>
    ...
    <b-modal :active.sync="isModalActive" has-modal-card :canCancel="!isLoading">
      <form @submit.prevent="selectedForum.id ? updateForum() : createForum()">
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">{{ modalTitle }} Forum</p>
          </header>
          ...
          <section class="modal-card-body">
            ...
            >
              <b-input
                v-model="selectedForum.url_media"
                v-validate="'required|max:255'"
                name="url_media"
                autofocus
              />
            ...
            >
              <b-select
                name="topics"
                v-model="selectedForum.topic_id"
                v-validate="'required'"
            ...
            >
              <b-select
                name="search_to"
                v-model="selectedForum.search_to"
                v-validate="'required'"
            ...
            <div class="mb-6">
              ...
              <ckeditor :editor="editor" v-model="selectedForum.body" :config="editorConfig"></ckeditor>
            </div>
          ...
          <footer class="modal-card-foot">
            ...
            <button 
              ...
              :disabled="isLoading">{{ modalTitle }} forum</button>
          </footer>
        </div>
      </form>
    </b-modal>
...
export default {
  ...
  computed: {
    ...mapState('forums', ['forums', 'isLoading']),
    ...mapState('topics', ['topics']),
    ...mapGetters('forums', ['search']),
    ...
    modalTitle: function() {
      return this.selectedForum.id ? "Update" : "Create";
    }
  },
  methods: {
    ...
    confirmDelete(forum) {
      ...
      this.$dialog.confirm({
        message: `Are you sure you want to delete "${this.selectedForum.title}"?`,
        confirmText: "Yes, I'm sure",
        type: 'is-success',
        hasIcon: true,
        onConfirm: this.deleteForum,
      });
    },
    ...
    showUpdateModal(forum) {
      this.selectedForum = Object.assign({}, forum);
      this.isModalActive = true;
    },
    ...
    async createForum() {
      try {
        this.selectedForum.type = "forum"; // store forum type blogs
        await this.store(this.selectedForum);
    ...
    async updateForum() {
      try {
        this.selectedForum.type = "forum"; // update forum type blogs
        await this.update(this.selectedForum);
    ...
    async deleteForum() {
      ...
      await this.destroy(this.selectedForum);
    },
    defaultImg(event){
